...
class VDPWIModel(nn.Module):
    ...
    def compute_sim_cube(self, seq1, seq2):
        ...
        for i in range(seq1.size(0)):
            min_value = torch.min(sim_cube[i, 0:12])
            max_value = torch.max(sim_cube[i, 0:12])
            sim_cube[i, 0:12] = (sim_cube[i, 0:12] - min_value) / (max_value - min_value + 1E-5)
    ...
    def compute_tree_sim_cube(self, seq1, seq2):
        ...
        sim_cube[:, 0:3] = compute_prism(seq1, seq2)
        for i in range(seq1.size(0)):
            min_value = torch.min(sim_cube[i, 0:3])
            max_value = torch.max(sim_cube[i, 0:3])
            sim_cube[i, 0:3] = (sim_cube[i, 0:3] - min_value) / (max_value - min_value + 1E-5)
        return sim_cube
    ...
    def forward(self, sent1, sent2, ext_feats=None, word_to_doc_count=None, raw_sent1=None, raw_sent2=None):
        ...
        # The computation of treenode-to-treenode matrix part
        # Use TreeLSTM and Dependency parser parent node
        l_hidden = []
        r_hidden = []
        seq_len_1 = sent1.size()[1]
        seq_len_2 = sent2.size()[1]
        rnn_hd = seq1f.size()[2]
        ...
        def findAllIndex(input_list, target):
            return list(np.where(np.array(input_list)==target)[0])
        ...
        for i in range(len(raw_sent1)):
            ...
            # put the hidden state in place of the original sentence order
            lhidden, l_idx_track = self.treeLSTM(ltree, linput, _sent1.split()) # seq_len: (state, hidden)
            self.treeLSTM.hidden_unit = []
            self.treeLSTM.idx_track = []
            l_reorder = list(zip(map(int, l_idx_track), lhidden))
            l_reorder.sort(key=takefirst)
            _, lhidden_sorted = zip(*l_reorder)
            lhidden_sorted = list(lhidden_sorted)
            # combine hidden state of word which belong to the same tree node
            for k in range(len(lhidden_sorted)):
                target = l_parent[k]
                all_others = findAllIndex(l_parent, target)
                all_others.remove(k)
                if all_others != []:
                    for ele in all_others:
                        lhidden_sorted[k] += lhidden_sorted[ele]
                    lhidden_sorted[k] = lhidden_sorted[k] / (len(all_others) + 1)
            for _ in range(seq_len_1 - len(lhidden_sorted)):
               lhidden_sorted.append(torch.zeros(rnn_hd).type(torch.cuda.FloatTensor))


            rhidden, r_idx_track = self.treeLSTM(rtree, rinput, _sent2.split())
            self.treeLSTM.hidden_unit = []
            self.treeLSTM.idx_track = []
            r_reorder = list(zip(map(int, r_idx_track), rhidden))
            r_reorder.sort(key=takefirst)
            _, rhidden_sorted = zip(*r_reorder)
            rhidden_sorted = list(rhidden_sorted)
            # combine hidden state of word which belong to the same tree node
            for k in range(len(rhidden_sorted)):
                target = r_parent[k]
                all_others = findAllIndex(r_parent, target)
                all_others.remove(k)
                if all_others != []:
                    for ele in all_others:
                        rhidden_sorted[k] += rhidden_sorted[ele]
                    rhidden_sorted[k] = rhidden_sorted[k] / (len(all_others) + 1)
            for _ in range(seq_len_2 - len(rhidden_sorted)):
                rhidden_sorted.append(torch.zeros(rnn_hd).type(torch.cuda.FloatTensor))

            l_hidden.append(torch.stack(lhidden_sorted, dim=0))
            r_hidden.append(torch.stack(rhidden_sorted, dim=0))

            del lhidden_sorted, rhidden_sorted
            del l_reorder, l_idx_track, r_reorder, r_idx_track, lhidden, rhidden

        seq1_tree = torch.stack(l_hidden, dim=0)  # [batch_size, seq_len, rnn_hd]
        seq2_tree = torch.stack(r_hidden, dim=0)

        sim_cube_tree = self.compute_tree_sim_cube(seq1_tree, seq2_tree)
        truncate = self.classifier_net.input_len
        ...
        focus_cube_tree = self.compute_tree_focus_cube(sim_cube_tree, pad_cube, l_parent_list, r_parent_list)

        final_cube = torch.cat((focus_cube, focus_cube_tree), 1)

        log_prob = self.classifier_net(final_cube)
        return log_prob
