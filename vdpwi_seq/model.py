...
class VDPWIConvNet(nn.Module):
    def __init__(self, config):
        ...
        def make_conv(n_in, n_out):
            conv = nn.Conv2d(n_in, n_out, 3, padding=1) # n_out: num of filters; kernal size: 3
...
class VDPWIResConvNet(nn.Module):
    def __init__(self, config):
        super().__init__()
        def make_conv(n_in, n_out):
...
class VDPWIModel(nn.Module):
    def __init__(self, dim, config):
        ...
        if config['classifier'] == 'vdpwi':
            self.classifier_net = VDPWIResConvNet(config)
        elif config['classifier'] == 'resnet':
            self.classifier_net = ResNet
    ...
    def forward(self, sent1, sent2, ext_feats=None, word_to_doc_count=None, raw_sent1=None, raw_sent2=None, visualize=False):
        ...
        # For Layer 1
        # previous sent1: [seq_len, embed_dim, batch_size]
        sent1 = sent1.permute(0, 2, 1).contiguous() #[seq_len, batch_size, embed_dim]
        sent2 = sent2.permute(0, 2, 1).contiguous()
        seq1f, _ = self.rnn(sent1) #seq1f: [seq_len, batch_size, hidden_size]
        seq2f, _ = self.rnn(sent2)
        seq1b, _ = self.rnn(torch.cat(sent1.split(1, 1)[::-1], 1)) #reverse the sent, then LSTM
        ...
        # seq1 = seq1_layer2_out
        # seq2 = seq2_layer2_out
